...
def get_remote_machine_infor():
    ...
    for porta in portas:
        ...
        if result == 0:
            ...
            print("[*] Porta %s não encontrada." % porta)
        s.close()
...
def scan_mac():
    ip = options.getmac
    arp_request = scapyy.ARP(pdst=ip)
    broadcast = scapyy.Ether(dst="ff:ff:ff:ff:ff:ff")
    arp_request_broadcast = broadcast/arp_request
    answered_lists = scapyy.srp(arp_request_broadcast,timeout=1,verbose=False)[0]
